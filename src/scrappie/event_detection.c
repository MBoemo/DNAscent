 ...
 **/
void compute_sum_sumsq( double *data, double *sum, double *sumsq, size_t d_length ) {
 ...
 **/
float *compute_tstat(const double *sum, const double *sumsq, size_t d_length, size_t w_length) {
	assert(d_length > 0);
	assert(w_length > 0);
 ...
 **/
size_t *short_long_peak_detector(DetectorPtr short_detector, DetectorPtr long_detector, const float peak_height) {
 ...
 **/
event_s create_event(size_t start, size_t end, double const *sums, double const *sumsqs, size_t nsample) {
...
void detect_events( double *raw, size_t raw_size, detector_param const edparam, event_s *allEvents ) {
	...
	compute_sum_sumsq(raw, sums, sumsqs, raw_size);
	float *tstat1 = compute_tstat(sums, sumsqs, raw_size, edparam.window_length1);
	float *tstat2 = compute_tstat(sums, sumsqs, raw_size, edparam.window_length2);

	Detector short_detector = {
		.DEF_PEAK_POS = -1,
		.DEF_PEAK_VAL = FLT_MAX,
		.signal = tstat1,
		.signal_length = raw_size,
	...
	allEvents[0] = create_event(0, peaks[0], sums, sumsqs, raw_size);
	// Other events -- peak[i-1] -> peak[i]
	for(size_t ev=1 ; ev < n - 1 ; ev++){
		allEvents[ev] = create_event(peaks[ev - 1], peaks[ev], sums, sumsqs, raw_size);
	}
	// Last event -- ends at nsample
	allEvents[n - 1] = create_event(peaks[n - 2], raw_size, sums, sumsqs, raw_size);
